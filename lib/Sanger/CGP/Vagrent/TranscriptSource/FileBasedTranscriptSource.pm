...
const my $DEFAULT_SEARCH_BUFFER => 10000;
...
sub getSearchPadding {
	return shift->{'_search_pad'};
}
...
sub _getTranscriptsFromCache {
  ...
  my $min;
  my $max = $gp->getMaxPos + $self->getSearchPadding;
  if($gp->getMinPos() < $self->getSearchPadding){
    $min = 0;
  } else {
    $min = ($gp->getMinPos - $self->getSearchPadding) - 1;
  }
...
sub _init {
  ...
  foreach my $k(keys(%vars)){
    ...
    } elsif($k eq 'search_pad'){
    	$self->{'_search_pad'} = $vars{$k};
    }
  }
  $self->{'_search_pad'} = $DEFAULT_SEARCH_BUFFER unless exists $self->{'_search_pad'} && defined $self->{'_search_pad'};
